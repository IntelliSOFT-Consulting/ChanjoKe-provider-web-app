...
function organizeData(items) {
  if (Array.isArray(items)) {
    const filteredItems = items.filter(
      (item) =>
        item?.type?.coding?.[0]?.display !== 'SYSTEM_GENERATED' ||
        item.system !== 'system-creation'
    )
    const hierarchy = {
    ...
    filteredItems.forEach((item) => {
      if (
        item?.type?.coding?.[0]?.display === 'Nemis' ||
        item?.type?.text === 'NEMIS No' ||
        item.system === 'NEMIS No' ||
        item.system === 'Nemis' ||
        item?.type?.coding?.[0]?.display === 'NEMIS No' ||
        item?.type?.coding?.[0]?.code === 'nemis'
      ) {
        hierarchy['NEMIS_NUMBER'] = item.value
      } else if (
        item?.system === 'Passport' ||
        item?.type?.coding?.[0]?.display === 'Passport'
      ) {
        hierarchy['PASSPORT'] = item.value
      } else if (
        item?.type?.coding?.[0]?.display === 'Birth Certificate' ||
        item?.type?.coding?.[0]?.display === 'Birth_Notification_Number' ||
        item?.type?.coding?.[0]?.display === 'Birth_Certificate' ||
        item?.type?.coding?.[0]?.display === 'Birth_certificate' ||
        item?.type?.coding?.[0]?.display === 'Birth_notification_number' ||
        item?.type?.coding?.[0]?.display === 'Birth Notification Number' ||
        item?.system === 'Birth Notification Number' ||
        item?.system === 'Birth Certificate' ||
        item?.type?.coding?.[0]?.display === 'birth_notification_number' ||
        item?.type?.coding?.[0]?.display === 'birth_certificate'
      ) {
        hierarchy['BIRTH_CERTIFICATE'] = item.value
      } else if (
        item?.system === 'National ID' ||
        item.system === 'ID Number' ||
        item?.type?.coding?.[0]?.display === 'ID Number' ||
        item?.type?.coding?.[0]?.display === 'ID_number'
      ) {
...
function deconstructPatientData(data, searchType) {
  ...
  return {
    ...
    idNumber:
      organizeData(data?.resource?.identifier).NATIONAL_ID ||
      organizeData(data?.resource?.identifier).BIRTH_CERTIFICATE ||
      organizeData(data?.resource?.identifier).NEMIS_NUMBER ||
      organizeData(data?.resource?.identifier).PASSPORT,
