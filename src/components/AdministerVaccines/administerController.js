  ...
  selectedVaccines.forEach((selectedVaccine) => {
    ...
    if (currentVaccine) {
      ...
      formatted.forEach((dependentVaccine) => {
        ...
        const nextEarliestDate = startDate.add(dependentVaccine.nextDose, 'days').format('YYYY-MM-DD');
        const nextLatestDate = moment(nextEarliestDate).add(difference, 'days').format('YYYY-MM-DD');
      ...
      formatted.forEach((dependentVaccine) => {
        const vaccineIndex = recommendations.findIndex((recommendation) =>
          recommendation.vaccineCode[0].coding[0].display === dependentVaccine.vaccineCode
        );

        if (vaccineIndex !== -1) {
          recommendations[vaccineIndex].dateCriterion = [
            { code: { coding: [{ code: 'Earliest-date-to-administer', display: 'Earliest-date-to-administer' }] }, value: dependentVaccine.earliestDate },
            { code: { coding: [{ code: 'Latest-date-to-administer', display: 'Latest-date-to-administer' }] }, value: dependentVaccine.latestDate }
          ];
        }
      });
    }
